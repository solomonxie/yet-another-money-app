...
export const secureStore = {
  ...
  setAiApiKey: (value: string) => SecureStore.setItemAsync(AI_API_KEY, value),
  ...
  setS3Credentials: (accessKeyId: string, secretAccessKey: string) =>
    Promise.all([
      SecureStore.setItemAsync(S3_ACCESS_KEY_ID, accessKeyId),
      SecureStore.setItemAsync(S3_SECRET_ACCESS_KEY, secretAccessKey),
    ]),
  clearS3Credentials: () =>
    Promise.all([
