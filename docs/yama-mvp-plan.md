...
- [ ] T9.2 Real iCloud Drive dropped: no first-party Expo module — needs a native module, a dev-client build, and a paid Apple Developer account. Re-scoped to **Google Drive** as the second cloud provider — moved to Backlog (see below), blocked on a user-owned Google Cloud Console OAuth Client ID.
